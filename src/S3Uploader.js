...
const S3Uploader = () => {
  ...
  useEffect(() => {
    async function fetchBuckets() {
      try {
        const response = await axios.get('http://100.26.109.158:3000/buckets');
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post(`http://100.26.109.158:3000/buckets/${selectedBucket}/upload`, formData, {
  ...
  const handleBucketChange = async (bucketName) => {
    ...
    try {
      const response = await axios.get(`http://100.26.109.158:3000/buckets/${bucketName}`);
